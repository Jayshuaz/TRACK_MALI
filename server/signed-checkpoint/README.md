...
- POST /api/v1/telemetry/checkpoint
  - Accepts a JSON body `{ events: [ ... ] }` and falls back to `src/sample_events.json` when no events are supplied.
...
Example requests
- Create checkpoint (uses sample events when body provides no events):
